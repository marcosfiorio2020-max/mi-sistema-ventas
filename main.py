...
def inicializar_base_datos():
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS usuarios 
                      (id SERIAL PRIMARY KEY, usuario TEXT UNIQUE, password TEXT, empresa_id TEXT)''')
    
    cursor.execute('''CREATE TABLE IF NOT EXISTS maestro_productos 
                      (id TEXT PRIMARY KEY, nombre TEXT, precio NUMERIC, unidad TEXT, categoria TEXT, stock NUMERIC, costo NUMERIC, empresa_id TEXT)''')
    
    cursor.execute('''CREATE TABLE IF NOT EXISTS ventas_cabecera 
                      (id SERIAL PRIMARY KEY, fecha TEXT, metodo_pago TEXT, total NUMERIC, empresa_id TEXT)''')
    
    cursor.execute('''CREATE TABLE IF NOT EXISTS ventas_detalle 
                      (id SERIAL PRIMARY KEY, venta_id INTEGER, producto TEXT, cantidad NUMERIC, unidad TEXT, precio_unitario NUMERIC, subtotal NUMERIC, costo_unitario NUMERIC, empresa_id TEXT)''')
    
    cursor.execute('''CREATE TABLE IF NOT EXISTS configuracion 
                      (id INTEGER PRIMARY KEY, meta_diaria NUMERIC, meta_mensual NUMERIC, empresa_id TEXT)''')
                      
    cursor.execute('''CREATE TABLE IF NOT EXISTS calendario_financiero 
                      (id SERIAL PRIMARY KEY, fecha TEXT, tipo TEXT, monto NUMERIC, descripcion TEXT, estado TEXT, empresa_id TEXT)''')
...
def registro(user: UsuarioLogin):
    ...
    try:
        cursor.execute("INSERT INTO usuarios (usuario, password, empresa_id) VALUES (%s, %s, %s)", (user.usuario, hash_pw, empresa_id))
        conexion.commit()
...
def login(user: UsuarioLogin):
    ...
    cursor = conexion.cursor()
    hash_pw = hashlib.sha256(user.password.encode()).hexdigest()
    cursor.execute("SELECT empresa_id FROM usuarios WHERE usuario = %s AND password = %s", (user.usuario, hash_pw))
    resultado = cursor.fetchone()
    cursor.close()
    conexion.close()
    if resultado:
        return {"status": "success", "empresa_id": resultado[0]}
    else:
...
def guardar_producto(p: Producto):
    ...
    cursor.execute(sql, (p.id, p.nombre, p.precio, p.unidad, p.categoria, p.stock, p.costo, p.empresa_id))
    conexion.commit()
    cursor.close()
    conexion.close()
    return {"status": "success"}
...
def obtener_catalogo(empresa_id: str):
    conexion = psycopg2.connect(URL_BASE_DATOS)
    cursor = conexion.cursor()
...
def ingresar_stock(ingreso: IngresoStock, empresa_id: str):
    conexion = psycopg2.connect(URL_BASE_DATOS)
    cursor = conexion.cursor()
    cursor.execute("UPDATE maestro_productos SET stock = stock + %s, precio = %s, costo = %s WHERE id = %s AND empresa_id = %s", 
                   (ingreso.cantidad, ingreso.nuevo_precio, ingreso.nuevo_costo, ingreso.id_producto, empresa_id))
    conexion.commit()
    cursor.close()
    conexion.close()
    return {"status": "success"}
...
def eliminar_producto(id_producto: str, empresa_id: str):
    conexion = psycopg2.connect(URL_BASE_DATOS)
    cursor = conexion.cursor()
        ...
        for d in venta.detalles:
            ...
            cursor.execute(
                '''INSERT INTO ventas_detalle 
                   (venta_id, producto, cantidad, unidad, precio_unitario, subtotal, costo_unitario, empresa_id) 
                   VALUES (%s, %s, %s, %s, %s, %s, %s, %s)''',
                (venta_id, d.nombre, d.cantidad, d.unidad, d.precio, d.subtotal, costo_u, empresa_id)
            )
            
            if not d.esManual:
                cursor.execute(
                    "UPDATE maestro_productos SET stock = stock - %s WHERE nombre = %s AND empresa_id = %s",
                    (d.cantidad, d.nombre, empresa_id)
                )
        conexion.commit()
        return {"status": "success"}
...
def obtener_ventas_admin(empresa_id: str):
    conexion = psycopg2.connect(URL_BASE_DATOS)
    cursor = conexion.cursor()
...
def obtener_dashboard(empresa_id: str):
    ...
    cursor.execute('''SELECT producto, SUM(subtotal - (cantidad * costo_unitario)) as ganancia 
                      FROM ventas_detalle WHERE empresa_id = %s GROUP BY producto ORDER BY ganancia DESC LIMIT 5''', (empresa_id,))
...
def actualizar_configuracion(config: Configuracion, empresa_id: str):
    ...
    if existe:
        cursor.execute("UPDATE configuracion SET meta_diaria = %s, meta_mensual = %s WHERE empresa_id = %s", 
                       (config.meta_diaria, config.meta_mensual, empresa_id))
    else:
        cursor.execute("SELECT COALESCE(MAX(id), 0) + 1 FROM configuracion")
        nuevo_id = cursor.fetchone()[0]
        cursor.execute("INSERT INTO configuracion (id, meta_diaria, meta_mensual, empresa_id) VALUES (%s, %s, %s, %s)", 
                       (nuevo_id, config.meta_diaria, config.meta_mensual, empresa_id))
    conexion.commit()
    cursor.close()
    conexion.close()
    return {"status": "success"}
...
def obtener_calendario(empresa_id: str):
    conexion = psycopg2.connect(URL_BASE_DATOS)
    cursor = conexion.cursor()
...
def agregar_evento(evento: EventoCalendario, empresa_id: str):
    conexion = psycopg2.connect(URL_BASE_DATOS)
    cursor = conexion.cursor()
    cursor.execute("INSERT INTO calendario_financiero (fecha, tipo, monto, descripcion, estado, empresa_id) VALUES (%s, %s, %s, %s, 'Pendiente', %s)", 
                   (evento.fecha, evento.tipo, evento.monto, evento.descripcion, empresa_id))
    conexion.commit()
    cursor.close()
    conexion.close()
    return {"status": "success"}
...
def saldar_evento(id_evento: int, empresa_id: str):
    conexion = psycopg2.connect(URL_BASE_DATOS)
    cursor = conexion.cursor()
...
def estado_resultados(inicio: str, fin: str, empresa_id: str):
    conexion = psycopg2.connect(URL_BASE_DATOS)
    cursor = conexion.cursor()
    ...
    cursor.execute('''SELECT SUM(monto) FROM calendario_financiero WHERE tipo = 'Pago' AND estado = 'Saldado' AND fecha >= %s AND fecha <= %s AND empresa_id = %s''', (inicio, fin_completo, empresa_id))
